...
#define JOYSTICK_X_ZERO 512
#define JOYSTICK_Y_ZERO 512
...
{ // Checks the current state of button Z
    uint16_t val = analogRead(JOYSTICK_BTNS);
...
#define JOYSTICK_READ_DELAY 50 // read joystick every 50 msec (about)
...
#ifdef DEBUG_JOYSTICK
            Serial.print("X analog value = ");
            Serial.println(xval);
            Serial.print("Y analog value = ");
            Serial.println(yval);
#endif // DEBUG 
            if (xval < 0.7*JOYSTICK_X_ZERO)
            {
                moveX = -1;
            }
            else if (xval > 1.3*JOYSTICK_X_ZERO)
            {
                moveX = 1;
            }
            if (yval < 0.7*JOYSTICK_Y_ZERO)
            {
                moveY = -1;
            }
            else if (yval > 1.3*JOYSTICK_Y_ZERO)
            {
                moveY = 1;
            }
        }
